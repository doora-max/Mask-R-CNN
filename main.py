...
parser.add_argument("--show-fps",default=True,help="display processing speed (fps)")
labels_group.add_argument("--text-thickness",default=1,type=int,help="thickness of label text")
...
def detect(image):
    ...
    output = model(torch.tensor(np.expand_dims(inp,axis=0),device="cuda").permute(0,3,1,2) / 255)[0]
    ...
    for box, label, score, mask in zip(*output.values()):
        ...
        if not args.hide_boxes:
            image = cv2.rectangle(image,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),(0,255,0)if classes[label]=='person'else colours[label], args.box_thickness)
        # draw label and score
        if not args.hide_labels:
            image = cv2.putText(image, classes[label]+str(round(score.item(),2)), (int(box[0]),int(box[1]) - 3),0, 1.5, (0,255,0)if classes[label]=='person'else colours[label], args.text_thickness)
        # draw mask
